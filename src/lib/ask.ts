...
function buildSystemPrompt(profileId: string): string {
  ...
  return [
    ...
    '  artists_cache(id text  -- spotify artist id, name text, image text)',
    '',
    'RULES:',
    ...
    '- Genres do NOT exist in this data — Spotify removed them from its API (the artists_cache.genres column survives but is always empty). Say you can’t answer genre questions rather than querying it and reporting an empty result as a real answer.',
