...
- A **standalone Cloudflare Worker** (`workers/sync/`) runs the 2-hourly cron: ingest recent plays, drain the import-enrichment backlog, then backfill missing artist artwork.
...
| C11 | **`genres`, `followers` and `popularity` removed from the artist object.** `/v1/artists/{id}` now returns only `id`, `name`, `images`, `href`, `uri`, `external_urls` (verified 2026-07-25 against 30 artists — every one came back with no genres). This is broader than C7, which only recorded `popularity` and the user fields | **No genre UI, ever** — same rule as audio features (C4). `artists_cache.genres` is kept as a column but is permanently empty; nothing may branch on it. Artist metadata worth caching is now name + image only |
...
- **Scopes:** `user-read-recently-played`, `playlist-read-private` (read-only, unchanged) plus exactly one write scope, `playlist-modify-private`, added for the `/ask` → Playlist tab. It's used for exactly one thing: creating a **private** playlist in the user's own account when they tap "Save to Spotify" (`src/pages/api/playlist/save.ts`) — always user-initiated, never automatic. Wrapt never modifies an existing playlist, never follows/unfollows, never posts publicly. Do not add `playlist-modify-public` — that's a different privacy posture. `spotify_profiles.scopes` stores what Spotify actually granted (from the token response, not the requested constant), so stale connections made before this scope existed can be detected and prompted to reconnect rather than failing silently.
...
- **`artists_cache`** — name/image per Spotify artist id, fetched one-by-one (C6) and refreshed lazily. It has a `genres` column that is **always empty** (C11) — don't build on it. It is the **only** source of the leaderboard's artist artwork (`left join` in `leaderboard_top_artists`); a missing row renders the gradient tile, not a broken image. Two writers keep it filled: `syncArtistMetadata` (artists seen in *live* plays) and `backfillTopArtistImages` (the top of the 30d/6m/all-time boards, 25 per cron cycle) — the latter exists because artists known only from **imported history** are never seen live and would otherwise stay art-less forever. `scripts/artist-details.mjs --top N --cache` does the same job in bulk.
...
- **Nav** (`src/components/AppNav.astro`) is a mobile bottom bar / desktop left rail: **Home (`/`) · History (`/history`) · Artist (`/artist`) · Ask (`/ask`) · Create (`/create`) · Settings (`/settings`)**. Only pages that pass `nav` to `AppShell` show it; auth screens don't. `/import` is deliberately **not** in the nav — it's a one-off/occasional task, reachable from `/settings` (both the enrichment card's "Go to Import →" link and the empty-state prompt) and directly at the URL.
...
- **`/ask`** — natural-language questions about your own listening, answered via **model-generated text-to-SQL** (see the security-invariants note). Ephemeral chat UI (vanilla JS, same convention-bend as `/import`) and a per-day question cap. Flow: `src/pages/ask.astro` posts to `src/pages/api/ask.ts` → `runAskAgent` (`src/lib/ask.ts`) loops the model (≤4 queries) through the `query_database` tool → `run_ask_sql` RPC → answers from the rows, optionally with a result table. LLM access is wrapped in `src/lib/llm.ts` (Cloudflare Workers AI `AI` binding, `@cf/meta/llama-4-scout-17b-16e-instruct`; **OpenAI-style tool schema** — the native `{name,parameters}` form is rejected `8001 Invalid input`, and tool results are fed back as a `user` turn, not `role:'tool'`). Needs the `AI` binding added in the Cloudflare Pages dashboard (see README).
- **`/create`** — brief + familiarity dial (`My music`/`Mix`/`Discovery`) → an AI-drafted ~28-track candidate list is resolved one-by-one against the real Spotify Search API; only tracks that actually match render (`src/lib/playlist.ts`: `buildListeningProfile` → `draftCandidates` → `validateCandidates` → one backfill pass if under 20 resolve). A brief that only partially resolves still renders whatever matched, honestly flagged (`struggled`), rather than refusing outright — only a fully empty result is a hard fail. Same per-day cap mechanism as Ask (`src/lib/usage.ts`, `kind='playlist'`, 10/day). "Save to Spotify" (`src/pages/api/playlist/save.ts`) is the app's one write call — see the security-invariants scope note.
- **`/history`** — chronological, paginated play log with date-range + free-text filters and a hero stat row (`src/lib/history.ts`).
- **`/artist`** — single-artist deep dive reached by searching a name: all-time plays, minutes, first play ever (and which track), and biggest listening year (`src/lib/artist.ts`, `artist_search`/`artist_summary` RPCs). A single search match redirects straight to `?name=`; multiple matches show a pick-list. The dashboard's artist drill-down (`?artist=` on `/`) links here too once you're inside one artist's tracks.
- **`/settings`** — Spotify connection + "last upload / last sync / last sync that added songs" (all derived from existing columns), enrichment progress, Reconnect button, and the entry point into `/import` (see the Nav note above).
- **`/import`** — self-serve Extended Streaming History upload (zip/json parsed client-side with `fflate`, batched to `/api/import/*`). Not in the nav — see above.
