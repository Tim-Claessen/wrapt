...
- **Nav** (`src/components/AppNav.astro`) is a mobile bottom bar / desktop left rail: **Home (`/`) · Ask (`/ask`) · History (`/history`) · Artist (`/artist`) · Import (`/import`) · Settings (`/settings`)**. Only pages that pass `nav` to `AppShell` show it; auth screens don't.
...
- **`/ask`** — natural-language questions about your own listening, answered via **model-generated text-to-SQL** (see the security-invariants note). Ephemeral chat UI (vanilla JS, same convention-bend as `/import`), a two-tab `Ask` / `Playlist` switcher, and a per-day question cap. Flow: `src/pages/ask.astro` posts to `src/pages/api/ask.ts` → `runAskAgent` (`src/lib/ask.ts`) loops the model (≤4 queries) through the `query_database` tool → `run_ask_sql` RPC → answers from the rows, optionally with a result table. LLM access is wrapped in `src/lib/llm.ts` (Cloudflare Workers AI `AI` binding, `@cf/meta/llama-4-scout-17b-16e-instruct`; **OpenAI-style tool schema** — the native `{name,parameters}` form is rejected `8001 Invalid input`, and tool results are fed back as a `user` turn, not `role:'tool'`). Needs the `AI` binding added in the Cloudflare Pages dashboard (see README).
  - **Playlist tab** — brief + familiarity dial (`My music`/`Mix`/`Discovery`) → an AI-drafted ~28-track candidate list is resolved one-by-one against the real Spotify Search API and only tracks that actually match render (`src/lib/playlist.ts`: `buildListeningProfile` → `draftCandidates` → `validateCandidates` → one backfill pass if under 20 resolve). Same per-day cap mechanism as Ask (`src/lib/usage.ts`, `kind='playlist'`, 10/day). "Save to Spotify" (`src/pages/api/playlist/save.ts`) is the app's one write call — see the security-invariants scope note.
