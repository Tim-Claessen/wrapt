...
Signup was previously magic-link/OTP-based; replaced with email+password because Tim doesn't like magic links. `/api/auth/confirm` (`exchangeCodeForSession`) is unchanged and now serves as the PKCE code-exchange target for `/signup`'s email confirmation link (`emailRedirectTo`) instead of the old magic-link redirect — reused as-is since it's just a generic "exchange this code for a session" handler. Self-serve signup is open to anyone with the URL (no invite-code gate) since this is a private app whose link Tim controls; the actual admission control is the Spotify allowlist (C1), not Supabase account creation. **Outstanding manual step:** whether Supabase's "Confirm email" project setting is on determines whether `signUp` returns an active session immediately or requires clicking the confirmation email first — check the Supabase Dashboard Auth settings for current behavior; `/signup`'s client script handles either case (redirects straight to `/connect` if a session comes back, otherwise shows a "check your email" message), but nobody has verified which mode this project is actually in.
...
- **Outstanding manual step (blocks the sync cron and the Leaderboard):** the three new migrations (`20260708000000_plays.sql`, `...000001_artists_cache.sql`, `...000002_leaderboard_functions.sql`) have **not been applied** to the live Supabase database — same situation as `spotify_profiles` originally (see `db push` note above). Paste them into the Supabase Dashboard SQL editor, in that order, before deploying `workers/sync` or visiting the dashboard's Leaderboard section. Confirmed missing live: hitting the worker locally against the real DB currently fails with `column spotify_profiles.plays_cursor_after_ms does not exist`.
- **Outstanding manual step (secrets):** `workers/sync` is a separate Worker from the Pages app and has its own secret store — `wrangler secret put <NAME> -c workers/sync/wrangler.jsonc` for `SPOTIFY_CLIENT_ID`, `PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `TOKEN_ENC_KEY` (same values as the Pages secrets) before the first `npm run sync:deploy`. Local dev reads `workers/sync/.dev.vars` (gitignored, already populated on this machine from the root `.dev.vars`).
