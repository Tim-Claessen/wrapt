...
- **Nav** (`src/components/AppNav.astro`) is a mobile bottom bar / desktop left rail: **Home (`/`) · History (`/history`) · Import (`/import`) · Settings (`/settings`)**. Only pages that pass `nav` to `AppShell` show it; auth screens don't.
