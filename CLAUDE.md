...
- **Dashboard data:**
  ...
  - Top artists / top tracks → `/me/top/{type}?time_range=` (`short_term`/`medium_term`/`long_term`). **Note (2026-07-08 redesign):** the dashboard leaderboard no longer calls these — every window is now computed from the `plays` log (see "Plays history + Leaderboard"). The endpoints/helpers still exist for future use but the dashboard doesn't hit them.
...
2. **No snapshots** (Trends) — the real app diffs live via `hasMovement` on `LeaderboardResult` rather than weekly snapshots (see "Plays history + Leaderboard" below). Post-redesign (2026-07-08) `hasMovement` is false only for the `all` window (the sole window with no comparable prior period); instead of a separate banner, the hero's compare caption states "no prior period to compare" and the rankings sub-label reads "ranked by plays".
...
- **Imported history & `ms_played` (minutes).** `plays.ms_played` (`20260709000000_ms_played.sql`) is the actual listened-time per play, which the Extended Streaming History export carries and imports store, so minutes are real the instant history lands. Live rows leave it null and fall back to the track's full `duration_ms`; every minutes figure sums `coalesce(ms_played, duration_ms, 0)`. Import ingestion runs through the **`ingest_import_plays(jsonb)`** RPC (INSERT … ON CONFLICT DO UPDATE `ms_played`), which **must dedup the incoming batch by `(profile_id, played_at)`** — two plays sharing a second-precision timestamp otherwise throw *"ON CONFLICT DO UPDATE command cannot affect row a second time"* and 500 the batch, which silently halted every web re-upload partway until `20260709000003_ingest_dedup.sql` added `distinct on`. The original ~209k-row import predated `ms_played`; it was backfilled once (2026-07-09) via **`scripts/backfill-ms-played.mjs`** (reads an export `.zip`/folder, dedups client-side, batches through the RPC, prints corrected minutes) — the reliable path when the web `/import` upload is flaky. Idempotent; safe to re-run.
...
- All 10 migrations — `20260707000000_spotify_profiles.sql` through `20260708000004_listening_stats.sql` (the first 6) plus the four from the 2026-07-08/09 dashboard redesign — are **applied** to the live Supabase database, confirmed by Tim 2026-07-09. The four redesign migrations: `20260709000000_ms_played.sql` (adds `plays.ms_played` + the `ingest_import_plays` RPC), `20260709000001_artist_rankings_by_name.sql` (name-based artist ranking), `20260709000002_leaderboard_minutes_and_drill.sql` (DROP+CREATEs the three leaderboard functions to add `total_ms` per-row minutes + a `p_artist` filter for the drill-down; re-includes 000001's logic so it supersedes it), `20260709000003_ingest_dedup.sql` (dedups `ingest_import_plays` — see the `ms_played` bullet above). Keep pasting new ones into the Supabase Dashboard SQL editor, in filename order, until the CLI is linked (see `db push` note above).
...
- **`src/lib/leaderboard.ts`** powers the dashboard's Leaderboard section (`src/pages/index.astro`). **As of the 2026-07-08 redesign, every window is computed from `plays`** — the Spotify-native windows are gone. Windows are `7d`/`30d`/`6m`/`all`/`custom` (the old `4w` and `lifetime` were dropped; `all` = all history, and is the one window with no prior-period movement). All windows slice `plays` via the Postgres RPCs (`supabase/migrations/20260708000002_leaderboard_functions.sql`) and diff against the immediately-preceding equal-length window for real rank-change (NEW/▲/▼) — so even the 6-month view now shows movement, which the old native path couldn't. Trade-off: a user with **no imported history** gets sparse long windows (native Spotify used to give instant all-time depth); acceptable while it's Tim-first, revisit for Zoe. Artist rankings now group by **lowercased artist name** (`20260709000001_artist_rankings_by_name.sql`) so imported plays rank immediately — the old id-grouping collapsed all un-enriched imports into one bogus NULL-id artist. Genre cross-filtering (via `artists_cache`) still only covers plays whose artists have been enriched. Every row also carries `total_ms` (minutes, `sum(coalesce(ms_played, duration_ms, 0))`), rendered as `N plays · Xm` in `LeaderboardRow`. **Drill-down:** `?artist=<name>` reuses `leaderboard_top_tracks` with a `p_artist` filter to show one artist's tracks (with a back link); genre rows link to the genre-filtered artists list. The three leaderboard functions were DROP+CREATEd by `20260709000002` because adding those OUT columns/params changed their signatures. RPCs are `service_role`-only — call via `createSupabaseServiceClient()` after verifying the caller's session with the request-scoped client. The dashboard no longer needs a Spotify token at all (leaderboard is pure DB). The page itself (`src/pages/index.astro`) was rebuilt in the redesign: a minutes hero over an album-art mosaic, a #1 card, slicer-driven comparison labels, `TrendChart` (undistorted area chart) + a responsive `ListeningHeatmap`; `RecentlyPlayed`/`StatsRow` are no longer used (files still present).
